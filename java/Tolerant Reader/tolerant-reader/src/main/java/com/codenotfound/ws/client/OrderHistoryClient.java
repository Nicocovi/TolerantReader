...
public class OrderHistoryClient {
  ...
  @Autowired
  private XPathExpression orderNameXPath;
  
  @Autowired
  private XPathExpression orderDescriptionXPath;
  ...
  public OrderHistory getOrderHistory(String userId) throws IOException {
    ...
    List<Order> orders = orderXPath.evaluate(responseResult.getNode(), new NodeMapper<Order>() {
      ...
      public Order mapNode(Node node, int nodeNum) {
    	...
    	// get the orderId
        String orderDescription = orderDescriptionXPath.evaluateAsString(node);
        
        // create an order
        return new Order("", "", orderDescription);
      }
    });
